# The AVP network predicts the future state of the vehicle based on a given control action
import numpy as np
from config.config import *
from utils.avp_util import *


class AvpNet:
    def __init__(self, sess):
        self.lr = FLAGS.avp_lr  # learning rate
        self.batch_size = FLAGS.avp_batch_size  # batch_size
        self.sess = sess

        self.action_num = FLAGS.action_num  # 11
        self._build_net()

    # build the network architecture
    def _build_net(self):
        # input: image or state, v_label: real value, p_label: real possibly
    ...
    # input: obs(image), output: possibility, value
